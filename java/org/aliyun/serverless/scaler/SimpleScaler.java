...
public class SimpleScaler implements Scaler {
    ...
    public void Idle(Context ctx, SchedulerProto.IdleRequest request, StreamObserver<SchedulerProto.IdleReply> responseObserver) throws Exception {
        ...
        try {
            ...
            responseObserver.onCompleted();
            if (needDestroy) {
                deleteSlot(ctx, request.getAssigment().getRequestId(), slotId, instanceId,
                        request.getAssigment().getMetaKey(), "bad instance");
            }
        } catch (Exception e) {
    ...
    private void deleteSlot(Context context, String requestId, String slotId, String instanceId, String metaKey, String reason) {
    ...
    private void gcLoop() {
        ...
        TimerTask task = new TimerTask() {
            ...
            public void run() {
                mu.lock();
                idleInstances.forEach(new Consumer<Instance>() {
                    @Override
                    public void accept(Instance instance) {
                        if (instance != null) {
                            ...
                            if (idleDuration > config.getIdleDurationBeforeGC().toMillis()) {
                                ...
                                CompletableFuture.runAsync(() -> deleteSlot(Context.current(), UUID.randomUUID().toString(),
                                        instance.getSlot().getId(), instance.getID(), instance.getMeta().getKey(), reason));
